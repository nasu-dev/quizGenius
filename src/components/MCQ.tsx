...
const MCQ = ({ game }: Props) => {
  ...
  const { mutate: checkAnswer, isLoading: isChecking } = useMutation({ //回答チェック
    mutationFn: async () => { //回答チェックAPIを呼び出す
      const payload: z.infer<typeof checkAnswerSchema> = { //回答チェックAPIのリクエストボディ
  ...
  const { mutate: endGame } = useMutation({ //ゲーム終了
    mutationFn: async () => { //ゲーム終了APIを呼び出す
      const payload: z.infer<typeof endGameSchema> = { //ゲーム終了APIのリクエストボディ
  ...
  const handleNext = React.useCallback(() => { //次へボタンの処理
    checkAnswer(undefined, { 
      onSuccess: ({ isCorrect }) => { 
        if (isCorrect) { //正解の場合
          setStats((stats) => ({ //統計情報を更新
            ...stats, 
    ...
    return (
      <div className="absolute flex flex-col justify-center -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2">
        <div className="px-4 py-2 mt-2 font-semibold text-white bg-green-500 rounded-md whitespace-nowrap">
          回答時間：{" "}
          {formatTimeDelta(differenceInSeconds(now, game.timeStarted))}  {/* 回答時間 */}
        </div>
        <Link
  ...
  return (
    <div className="absolute -translate-x-1/2 -translate-y-1/2 md:w-[80vw] max-w-4xl w-[90vw] top-1/2 left-1/2"> 
      ...
      <div className="flex flex-col items-center justify-center w-full mt-4">
        ...
        >
          {isChecking && <Loader2 className="w-4 h-4 mr-2 animate-spin" />} {/* ローディングアイコン */}
